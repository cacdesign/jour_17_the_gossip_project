...
# - Je crée mes hashtags
10.times do |index|
	h = Hashtag.create
	p "Un hashtag a été crée"
end


# - Je crée mes gossips
10.times do |index|
	g = Gossip.create(title: Faker::HarryPotter.spell, content: Faker::HarryPotter.quote, user: User.all.sample, hashtag: Hashtag.all.sample)
	p "Le gossip #{g.title} avec le contenu #{g.content} a été créé avec le hashtag #{g.hashtag}"
end


# - Je crée mes tags
10.times do |index|
	t = Tag.create(title: Faker::Color.color_name, hashtag: Hashtag.all.sample)
	p "Le tag #{t.title} a été crée avec le hashtag #{t.hashtag}"
end
